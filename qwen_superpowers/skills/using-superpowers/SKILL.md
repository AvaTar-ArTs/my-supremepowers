...
> **Qwen adapter:** Qwen-local runtime copy. Author and maintain behavior directly in `~/.qwen`.
...
**In Qwen:** Use the native Skill tool/menu when available, or the Qwen orchestrator (`~/.qwen/scripts/qwen-sp use <name>`) when operating from the shell. When a skill is loaded, follow the loaded content directly.
...
| "We already used this skill earlier" | Re-check skill applicability per message. Prior use does not grant exemption. |
| "User wants speed, skip the skill check" | Speed is not a waiver. Run the skill check first, then move fast. |
| "I can do one command first and come back" | No pre-actions before skill check when applicability is non-zero. |
| "No planning loops, just output now" | You can skip long planning, but never skip the skill applicability gate. |
