...
> **Supremepower adapter:** Runtime adapter surface. Author active behavior in `~/.qwen` first, then sync to `~/.supremepower` as needed.
...
**In Qwen:** Use the native Skill tool/menu when available, or the Qwen orchestrator (`~/.qwen/scripts/qwen-sp use <name>`) when operating from shell.
